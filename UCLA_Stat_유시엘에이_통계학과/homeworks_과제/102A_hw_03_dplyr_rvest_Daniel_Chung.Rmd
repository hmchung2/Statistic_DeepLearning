...
```{r exc3}
load("C:/Users/hmchu/Downloads/dr4.Rdata")

dr4 %>% filter(is.na(visit2) == FALSE)%>% mutate(diff1 = visit2- visit1, diff2 = visit3- visit2, diff3 = visit4- visit3, diff4 = visit5- visit4)%>%select(id,diff1,diff2,diff3,diff4)%>% summarise(time = sum(diff1, diff2,diff3,diff4, na.rm = TRUE), visits = nrow(dr4)*4 - sum(is.na(dr4)),average = time/ visits )
